...
const page =() =>{
    ...
    useEffect(()=> {
      const fetchData= async() => {
        try{
          ...
          console.log('Response is positive',response);
    ...
    const filteredText= data.filter((prod)=> {
      const textBook= prod.name;
      if(selectedboxes.length==workbooks[0].items.length){
        return data;
      }else if(selectedboxes.length>0){
        return selectedboxes.some((selectedTB)=> textBook.includes(selectedTB))
      }else{
        return data;
      }
    })
  ...
  return (
    <div className='flex flex-col bg-white'>
        ...
        <div className='text-gray-500 mt-[180px] md:mt-[210px] mx-10'>
            <h2 className='font-bold text-2xl mb-5'>Textbooks</h2>
        
            <div className='flex flex-wrap mx-5 my-5'>
            {filteredText.map((prod) => (
